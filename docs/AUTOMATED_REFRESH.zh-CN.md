...
1. 定时任务或手动调度刷新 Provider 缓存，并运行各 Provider 的来源 Adapter。
2. `npm run check` 校验代码、预览、测试和生成 Provider 索引 schema v4；当前索引
   包含 7 个 Provider、109 条 style source 和 600 条 component source。
...
- `daisyui-themes` CSS 会先缩减为受治理的规范主题 JSON，再进入哈希与 Kimi 策展；
  仓库中的任意 CSS 不会被索引。
