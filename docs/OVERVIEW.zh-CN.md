...
schema v5 模型按 Direction 展示一张卡片，可切换关联 Theme，并支持文本搜索以及
体验类型、family、页面类型、信息密度、视觉调性和组件库过滤。无筛选的首批结果
按六类确定性轮转，搜索和筛选结果保持规范顺序。规范预览路径为
...
因此可以测试和复现。
中英文显式体验类型会参与评分；近分候选可以补充新的体验类型，但 Top 1 和明显
更强的候选由确定性阈值保护。

每个推荐包含带可见体验类型的 Direction、所选 Theme、对应 PreviewSpec，以及：
