...
- `catalog/generated/style-sources.json`：当前 74 条上游风格来源路径索引；
...
组件库标签和上游 style-source 数量构建 schema v2 目录视图模型。轻量
`/catalog.json` 不再内嵌 SVG，只为每个条目提供 `previewUrl`；服务在
`/previews/<style-id>.svg` 以独立同源路由提供已提交的中性预览，并限制为
Catalog 中经过校验的安全 style ID。
...
浏览器只绑定 `127.0.0.1`，并提供 `/`、`/catalog.json`、`/app.js`、
`/styles.css` 及上述同源预览路由。页面把 48 个已策展 profile 显示为完整
