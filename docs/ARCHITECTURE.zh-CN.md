...
- `catalog/recommendation-benchmarks.json`：保护 family/体验类型意图、相关性护栏与
  确定性排序的 12 个 schema v2 代表场景。
...
`src/core.mjs` 根据显式体验类型、family、页面类型、受众、产品目标、密度、
调性、关键词和适用场景，为 Direction 进行确定性加权匹配。共享 taxonomy 的
中英文别名可识别 `consumer app`/“C 端”、`B2B`/“B 端”和
`admin console`/“管理台”。程序会标准化词项、过滤过于通用的 brief 词，并只从
达到相关性阈值的 Direction 中做相关性优先的差异化。

Top 1 始终锁定原始最高分。未出现的体验类型和 family 只是软偏好：被提前的候选
既要达到当前最佳剩余分数的 80%，也要达到原始 Top 1 的 50%。系统不会为凑齐
体验类型使用硬配额。
...
不会在运行时自行判断或重排风格。schema v2 的 12 场景推荐 benchmark 会校验
family 与体验类型意图、Top 1、未入选的更高分候选，以及重复运行的确定性。
