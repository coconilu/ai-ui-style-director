...
标签和上游 style-source 数量构建 schema v4 目录视图模型。每个条目对应一个
...
模型还包含由规范 Direction、Theme、关联、PreviewSpec 与 alias 文档确定性
计算的 `catalogRevision`。CLI 会把本地预期 revision 附在 Pages URL 上；浏览器把它
...
托管浏览器视图模型也独立使用 schema v4。版本数字相同不代表两类契约可以混用。
